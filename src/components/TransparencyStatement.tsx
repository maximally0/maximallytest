  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-3 mb-6">
        <Sparkles className="h-8 w-8 text-maximally-purple animate-float" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {transparencyPoints.map((point, index) => (
          <div key={index} className="flex items-start space-x-3 group">
            <div className="rounded-full bg-gradient-to-r from-maximally-green to-maximally-blue p-1 mt-0.5 group-hover:animate-pulse transition-all duration-300">
              <Check className="h-5 w-5 text-white" />
            </div>
            <span className="text-gray-800 font-medium text-lg group-hover:text-maximally-purple transition-colors duration-300">{point}</span>
          </div>
        ))}
      </div>
    </div>
